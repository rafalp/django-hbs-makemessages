...
"applications) directory.\n\nIf command is executed for djangojs domain, it "
"also pulls strings from .hbs and .handlebars files.\n\nYou must run this "
"command with one of either the --locale, --exclude or --all options.")
    ...
    def handle(self, *args, **options):
        ...
        cdw = Path(os.getcwd())
        subdirs = [text_type(d.basename()) for d in cdw.dirs()]
        use_subroutines = 'locale' in subdirs and self.domain == 'djangojs'

        tmp_js_files = []
        if use_subroutines:
            # fake js files from templates
            tmp_js_files = self.prepare_tmp_js_files(cdw);
        ...
        if use_subroutines:
            ...
            self.cleanup_po_files(cdw, locales, tmp_js_files);
...
class Admin(ManagementUtility):
    """
    Thin wrapper for django-admin.py that swaps Django's makemessages for ours
    """
    def fetch_command(self, subcommand):
        if subcommand == "makemessages":
            return Command()
        else:
            return super(Admin, self).fetch_command(subcommand)
